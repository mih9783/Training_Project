...
// таймер
function timer() {
  var deadline = "2020-01-31"; // remaining - оставшийся

  function getTimeRemaining(endtime) {
    var t = Date.parse(endtime) - Date.parse(new Date()); // Date.parse(endtime) - это конечная дата
    // Date.parse(new Date()) - это дата на сейчас
    // получаем часы, минуты и секунды

    var seconds = Math.floor(t / 1000 % 60);
    var minutes = Math.floor(t / 1000 / 60 % 60);
    var hours = Math.floor(t / (1000 * 60 * 60)); // это количество дней
  ...
  function setClock(id, endtime) {
    ...
    function updateClock() {
      var t = getTimeRemaining(endtime); // чтобы цифры до десятки были с нулями
